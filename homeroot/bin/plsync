...
PHONE_ADDRESS="192.168.42.129"				# Address to access your phone over ssh for sync
PORT="22"
...
function sync() {
    rsync -vzrL --rsh='ssh -p '${PORT} --delete-after --exclude '*.cue' --stats --progress --no-perms --ignore-existing --no-t ${SYNC_MUSIC_ROOT}/ ${PHONE_ADDRESS}:${PHONE_MUSIC_ROOT}/ 
}
...
confirm "Would you like to sync with your phone? (Make sure sshd runs on the device)" &&
#  adb start-server &&
